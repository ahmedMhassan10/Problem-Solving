...
vll dpWays(N,0),dpMax(N,0),dpMin(N,OO);
void Dijkstra( ll src)
{
    ...
    while(!pq.empty())
    {
        ...
        for(auto ch : g[node])
        {
            ...
            {
                dpWays[ch.F]=dpWays[node];
                dpMax[ch.F]=dpMax[node]+1;
                dpMin[ch.F]=dpMin[node]+1;
                dist[ch.first] = dist[node] + ch.second;
                pq.push({-dist[ch.first],ch.first});
            }else if(dist[node] + ch.second == dist[ch.first]){
                dpWays[ch.F]=add(dpWays[ch.F],dpWays[node],mod);
                dpMax[ch.F]=max(dpMax[ch.F],dpMax[node]+1);
                dpMin[ch.F]=min(dpMin[node]+1,dpMin[ch.F]);
                dist[ch.first] = dist[node] + ch.second;
//                pq.push({-dist[ch.first],ch.first});
            }
        }
...
void func(int inde) {
    ...
    Dijkstra(1);
    cout<<dist[n]<<" "<<dpWays[n]<<" "<<dpMin[n]<<" "<<dpMax[n];
}
int main() {
