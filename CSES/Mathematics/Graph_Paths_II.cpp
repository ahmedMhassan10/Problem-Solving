...
const ll OO =2e18, mod = 1e9+7,mod2=1e9+9, N =5000+5,M=30,MOD= 998244353, minOO=LLONG_MIN;
...
vector<vector<int>> mul(vector<vector<int>> &a, vector<vector<int>> &b) {
    ...
    vector<vector<int>> c(n, vector<int>(m, OO));
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++)
            for (int k = 0; k < f; k++)
            {
...
void func(int index){
    ll n,m,k;cin>>n>>m>>k;
    vector<vector<int>>a(n,vector<int>(n,OO));
    for (int i = 0; i < m; ++i) {
        ll u,v;cin>>u>>v;
        ll w;
        cin>>w;
