...
int main() {
       // freopen("survive.in "," r ",stdin);
        ...
        map<char,bool>ma;
        fo(i,siz(str2)){
            if(str2[i]=='0')ma[i+'a']=true;
        }
        vi prefix(siz(str1)+4),prefix2(siz(str1)+4);
        make_hash(str1,prefix,pw,inv,mod);
        make_hash(str1,prefix2,pw2,inv2,mod2);
        set<pair<int,int>>s;
        fo(i,siz(str1)){
            int re=0;
            fp(j,i,siz(str1)-1){
                if(ma[str1[j]])re++;
                if(re>k)break;
                s.insert({get_value(i,j,prefix,pw,inv,mod),get_value(i,j,prefix2,pw2,inv2,mod2)});
            }
        }
        cout<<s.size()<<'\n';
        return 0;
